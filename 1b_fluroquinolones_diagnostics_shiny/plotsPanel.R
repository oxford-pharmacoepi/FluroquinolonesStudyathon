...
plotsPanelViewer <- function(id, title, height = "200px") {
  ...
  tabPanel(title,
           ...
           tags$hr(),
           withSpinner(plotOutput(ns('drugDaysSupplyPlot'), height = height)),
           withSpinner(plotOutput(ns('drugDurationPlot'), height = height)),
           withSpinner(plotOutput(ns('drugQuantityPlot'), height = height)))
}
  ...
  shiny::moduleServer(id,
                      function(input, output, session) {
                        ...
                        if (!is.null(plotData[[1]]) && nrow(plotData[[1]]) > 0) {
                          ...
                          output$drugDaysSupplyPlot <- renderPlot({
                            createBarChart(getDaysSupplyData())
                          })
                          output$drugDurationPlot <- renderPlot({
                            createBarChart(getDurationData())
                          })
                          output$drugQuantityPlot <- renderPlot({
